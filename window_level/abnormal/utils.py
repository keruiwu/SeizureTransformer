...
def prepare_TUSZ_dataset(root):
    ...
    # prepare training and test data loader
    train_dataset = IndexDataset(train_df, all_eegs, transform_fn)
    test_dataset = IndexDataset(test_df, all_eegs, transform_fn)
    val_dataset = IndexDataset(valid_df, all_eegs, transform_fn)
    # print(len(train_files), len(val_files), len(test_files))
    return train_dataset, test_dataset, val_dataset

def get_metrics(output, target, metrics, is_binary, threshold=0.6):
    if is_binary:
